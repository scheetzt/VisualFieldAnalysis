...
def readIntersectFile(mX, mY, ppd, isoIntercepts, nd):
	...
	for i in range(0,360):
		...
		extrap_III4e = rel_I4e * nd['III4e'][theta]
		extrap_V4e = rel_I4e * nd['V4e'][theta]
...
## Ignore this for now. Should be handled before we get here...
#	else:
#		complete_isopter = combineIsopters(isopters_open, isopters_extrap)
#		point_list = []
#		for i in range(0,len(complete_isopter),3):
#			for j in range(0,len(complete_isopter[i+1])):
#				point_list.append((complete_isopter[i+1][j], complete_isopter[i+2][j]))
#		polygon = Polygon(point_list)
	...
	#	x1 = mX
	#	y1 = mY
	#	x2 = (12*ppd*(math.cos(math.radians(i)))) + mX
	#	y2 = (12*ppd*(math.sin(math.radians(i)))) + mY
	#	#print("I=%4d, X=%0.2f, Y=%0.2f" % (i,x2,y2))
	## end testing
